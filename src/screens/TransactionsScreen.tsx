...
const TransactionsScreen: React.FC = () => {
  ...
  const [invoiceTransactionId, setInvoiceTransactionId] = React.useState<
    string | null
  >(null);
  ...
  const groupedTransactions = React.useMemo(() => {
    const filteredTransactions = transactions.filter((tx) => {
      ...
      if (endDate && txDate > new Date(endDate + "T23:59:59")) {
        return false;
      }
    ...
    <div className="min-h-screen p-4 sm:p-6">
      <header className="mb-6">
        <div className="flex justify-between items-center mb-4">
          <button
            ...
            className="p-2 rounded-full bg-light-secondary dark:bg-dark-secondary"
          >
            <ArrowLeft
              size={24}
        ...
        <div className="flex gap-3 mt-4">
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-light-secondary dark:bg-dark-secondary rounded-lg border border-light-border dark:border-dark-border focus:border-brand-green focus:outline-none text-sm"
            />
          </div>
          <div className="flex-1">
            <label className="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary mb-1">
          ...
          {(startDate || endDate) && (
            <button
              ...
              className="self-end px-3 py-2 text-red-400 hover:text-red-300 text-sm font-medium"
            >
              Clear
            </button>
          )}
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              filterType === "all"
                ...
                : "bg-light-secondary dark:bg-dark-secondary text-light-text-secondary dark:text-dark-text-secondary hover:bg-light-border dark:hover:bg-dark-border"
            }`}
          >
            All
          </button>
      ...
      {loading ? (
            ...
            {Object.entries(groupedTransactions).map(([date, txs]) => (
              ...
              >
                <div className="flex items-center mb-2 sm:mb-3">
                  <Calendar
                    size={16}
                    className="text-light-text-secondary dark:text-dark-text-secondary mr-2"
                  />
        ...
        <div className="text-center py-20">
          ...
          <Link
            to="/add-transaction"
            className="bg-brand-green text-white font-bold py-3 px-6 rounded-xl text-lg hover:bg-green-600 transition-colors"
          >
            Add Transaction
          </Link>
        </div>
      ...
      <TransactionInvoiceModal
        ...
        transaction={
          transactions.find((tx) => tx.id === invoiceTransactionId) || null
        }
        category={getCategory(
          transactions.find((tx) => tx.id === invoiceTransactionId)
            ?.category_id || ""
        )}
        onEdit={() => {
