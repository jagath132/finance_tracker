...
const TransactionDetailScreen: React.FC = () => {
    ...
    const amountColor = transaction.type === 'income' ? 'text-brand-green' : 'text-brand-red';
        ...
        <div className="min-h-screen bg-dark-primary text-white p-4 sm:p-6 lg:p-8 pb-20">
            <header className="flex items-center justify-between mb-8">
                ...
                <div className="flex gap-2">
                    <button onClick={() => transaction && openModal('editTransaction', transaction)} className="p-2 rounded-full bg-dark-secondary">
                        <Edit size={24} className="text-gray-400" />
                    </button>
                    <button onClick={() => setShowDeleteConfirm(true)} className="p-2 rounded-full bg-dark-secondary">
            ...
            <div className="bg-dark-secondary rounded-2xl p-6 shadow-lg">
                <div className="text-center border-b border-dashed border-gray-700 pb-6 mb-6">
                    ...
                    <p className={`text-5xl font-bold my-2 ${amountColor}`}>
                        ₹{new Intl.NumberFormat('en-IN').format(Math.abs(transaction.amount))}
                    </p>
                    <h2 className="text-2xl font-semibold">{transaction.description}</h2>
                    <p className="text-gray-500">{new Date(transaction.transaction_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                </div>

                <div className="space-y-4 text-sm mb-6">
                    <div className="flex justify-between">
                        <span className="text-gray-500">Category</span>
                        <span className="font-medium text-white">{getCategoryName(transaction.category_id)}</span>
                    </div>
                    {transaction.notes && (
                         <div className="flex justify-between text-right">
                            <span className="text-gray-500 flex-shrink-0 mr-4">Notes</span>
                            <span className="font-medium text-white">{transaction.notes}</span>
                        </div>
                    )}
                </div>
            </div>
